...
def parse_args():
    ...
    parser.add_argument("--train_adversaries", action="store_true", default=False)
    parser.add_argument("--train_positive_agent", action="store_true", default=True)
...
def train(arglist):
    with U.single_threaded_session():
        ...
        if arglist.restore:
            final_ep_rewards = list(np.load(arglist.plots_dir + arglist.exp_name + '_episode_rewards.npy'))
            final_ep_ag_rewards = list(np.load(arglist.plots_dir + arglist.exp_name + '_agent_rewards.npy'))
            final_ep_ag_rewards = [list(a) for a in final_ep_ag_rewards]
        else:
            final_ep_rewards = []  # sum of rewards for training curve
            # final_ep_ag_rewards = []  # agent rewards for training curve
            final_ep_ag_rewards = [[0.0] for _ in range(env.n)]  # agent rewards for training curve
        ...
        while True:
            ...
            if len(episode_rewards) > arglist.num_episodes:
                ...
                np.save(arglist.plots_dir + arglist.exp_name + '_agent_rewards.npy', agent_rewards)
                np.save(arglist.plots_dir + arglist.exp_name + '_episode_rewards.npy', episode_rewards)
